...
async function initializeModel() {
  try {
    ...
    if (!modelDetails) {
      throw new Error(
        `Model details not found for modelId: ${apiKeyRecord.modelId}`
      );
    }
  ...
  const promptTemplate = ChatPromptTemplate.fromMessages([
    {
      role: "system",
      content:
        "You are an assistant providing a comprehensive summary of a medical training session, including details on student interactions and predefined patient data.",
    },
    {
      role: "user",
      content: `
  ...
  const promptTemplate = ChatPromptTemplate.fromMessages([
    {
      role: "system",
      content:
        "You are a tutor evaluating a student's performance based on a medical training summary. You will evaluate the student based on specific scoring criteria and provide scores for each section, along with constructive feedback.",
    },
    {
      role: "user",
      content: `
