...
const patientController = {
        ...
        try {
            const patients = await Patient.findAll({
                include: [
                    ...
                    {
                        model: Prompt,
                        attributes: ['mandatoryQuestions', 'medicalHistory', 'predefinedTreatments'], // Include relevant fields from Prompt
                    },
                ],
            });
    ...
    async createPatient(req, res) {
        ...
        try {
            ...
            if (existingPatient) {
                return res.status(400).json({
                    message: 'A patient with the same name already exists. Please use a different name.',
                });
            }
            ...
            res.status(500).json({ error: 'An error occurred while creating the patient and prompt' });
        }
    },
